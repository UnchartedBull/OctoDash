...
export class UpdateComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (!this.service.getLatestVersion() || !this.service.getLatestVersionAssetsURL()) {
      this.notificationService.setWarning(
        $localize`Can't initiate update!`,
        $localize`Some information is missing, please try again in an hour or update manually.`,
      );
  ...
  private setupListeners(): void {
    this.electronService.ipcRenderer.on('updateError', (_, updateError: UpdateError): void => {
      this.notificationService.setError($localize`Can't install update!`, updateError.error.message);
      this.closeUpdateWindow();
    });
