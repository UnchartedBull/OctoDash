  ...
  private loadSpools(): void {
    this.filamentPluginService.getSpools().subscribe(
      ...
      (error: HttpErrorResponse): void => {
        this.notificationService.setError($localize`Can't load filament spools!`, error.message);
      },
      (): void => {
        this.loading = false;
      },
    ...
    this.filamentPluginService.getCurrentSpool().subscribe(
      ...
      (error: HttpErrorResponse): void => {
        this.notificationService.setError($localize`Can't load active spool!`, error.message);
      },
    );
  }
            ...
            (spoolRemote: FilamentSpool): void => {
              if (spool.id === spoolRemote.id) resolve();
                ...
                this.notificationService.setError(
                  $localize`Spool IDs didn't match`,
                  $localize`Can't change spool. Please change spool manually in the OctoPrint UI.`,
                );
                reject();
              }
            },
            (error): void => {
              this.notificationService.setError($localize`Can't set new spool!`, error.message);
              reject();
            },
          );
        },
