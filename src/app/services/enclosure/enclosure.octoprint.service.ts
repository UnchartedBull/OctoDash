...
export class EnclosureOctoprintService implements EnclosureService {
  ...
  setLEDColor(identifier: number, red: number, green: number, blue: number): void {
    ...
    this.http
      ...
      .pipe(catchError(error => this.notificationService.setError($localize`Can't set LED color!`, error.message)))
      .subscribe();
  }
  ...
  setOutput(identifier: number, status: boolean): void {
    ...
    this.http
      ...
      .pipe(catchError(error => this.notificationService.setError($localize`Can't set output!`, error.message)))
      .subscribe();
  }
  ...
  setPSUState(state: PSUState): void {
    if (this.configService.usePSUControl()) {
      ...
      this.notificationService.setWarning($localize`Can't change PSU State!`, $localize`No provider for PSU Control is configured.`);
    }
  }
